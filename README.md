...
已更新至 2023 年所有 Vue 3 新功能！加入拥有超过 90 万名开发者的在线社区，并参加由曾在硅谷和多伦多使用 Vue.js 进行过实际开发的行业专家授课的课程。**
...
本课程使用最新版本的 `Vue`，注重效率。从此告别令人困惑、过时或不完整的教程。**Andrei 的学员如今就职于 `Google`、特斯拉、亚马逊、苹果、IBM、摩根大通、Facebook 等顶尖科技公司。**
...
我们保证这是最全面的 `Vue` 在线资源。这门基于项目的课程将向您介绍 2023 年 `Vue` JS 开发人员所需的所有现代工具链。我们将使用 `Vue`、`Pinia`、`Vue Router`、`Composition API`、`Firebase`、`Vitest`、`Sass` 等工具构建一个类似于 `Spotify` 的大型音乐播放器应用程序。这将是一个使用 `Firebase` 的全栈应用程序。
...
所有代码都会逐步提供，即使您不喜欢跟着代码编写，您也可以访问完整的项目代码，因此任何报名参加课程的人都可以立即拥有自己的项目并将其添加到作品集中。**
...
本课程将非常注重实践，我们将带领你从头到尾完成一个专业的 `Vue` 项目，直至最终上线。我们将从最基础的 `Vue` 知识讲起，然后深入探讨高级主题，以便你在未来的任何 `VueJS` 项目中都能做出明智的架构和工具选择。无论你是前端框架新手，还是拥有 `React` 或 `Angular` 开发经验，你都会爱上 `Vue`！
...
**1. 基础知识 -** 在本节中，我们将学习 `Vue` 的基础知识，例如计算属性、方法和监听器。掌握这些基础知识后，我们将能够使用事件、绑定和条件渲染来操作文档。我们还将学习如何使用官方的 Vue 开发者工具来调试应用程序。
...
**2. 项目：透视演示平台 -** 在本节中，我们将构建第一个项目，创建一个 `CSS` 透视演示平台。这将使我们有机会通过开发一个工具来巩固上一节中学到的许多基本概念，该工具用于可视化 `CSS` 属性如何操纵元素的透视效果。
...
**3. 深入基础 -** 在本节中，我们将深入探讨 `Vue` 框架的内部运作机制。`JavaScript` 框架如何实现响应式？`Vue` 如何将模板编译成虚拟 `DOM` 对象？这将帮助我们了解 Vue 如何构建现代应用程序框架。我们还将讨论生命周期钩子和组件。
...
**4. 工具介绍 -** 在本节中，我们将暂时放下 `Vue`，来谈谈一些用于应用程序开发的工具。在你的职业生涯中，你将需要用到这些帮助我们开发应用程序的工具（例如 `Vite`、`Sass`、`PostCSS`、`ESLint`）。我们将讨论如何配置这些工具以及它们存在的必要性。

**5. ADVANCED COMPONENTS -** In this section, we'll scaffold a Vue project with `Vite`. The goal of `Vite` is to make the development experience as smooth and painless as possible. `Vite` can help us with scaffolding a project, configuring tools, and running a server. This alleviates a lot of the more tedious tasks of web development. We'll be able to jump straight into components to see how we can architect an application with components by using `props`, emitting events, slots, and dynamically rendering content.
**5. 高级组件 -** 在本节中，我们将使用 `Vite` 搭建一个 `Vue` 项目。`Vite` 的目标是让开发体验尽可能流畅轻松。`Vite` 可以帮助我们搭建项目、配置工具和运行服务器，从而减轻 `Web` 开发中许多繁琐的任务。我们将直接进入组件的世界，了解如何使用 `props`、事件触发、插槽和动态渲染内容来构建应用程序。
...
**6. 过渡效果和动画 -** 在本节中，我们将学习如何通过添加过渡效果和动画为应用程序增添活力。动画可以为任何应用程序注入生机，还可以通过向用户提供操作反馈来改善用户体验。我们将讨论如何使用 `CSS` 和 `JavaScript` 实现动画。`Vue` 也支持这两种语言。我们甚至还会介绍最新的 `Web Animations API`。
...
**7. 项目：测验应用 -** 在本节中，我们将运用目前为止所学的所有知识来开发一个测验应用。测验是吸引用户参与几乎所有行业的绝佳方式。
...
**8. 毕业设计：PINIA 入门 -** 在本节中，我们将开始本课程的毕业设计项目，这是一个音乐应用！您可以上传、管理和流畅地收听音乐。我们将使用 `Tailwind CSS` 来设计一个美观的现代应用。`Tailwind CSS` 是前端开发社区正在迅速崛起的新兴技术。之后，我们将学习如何使用官方状态库 `pinia` 进行状态管理（在新版本代码库中，`PINIA` 取代了 `Vuex`）。
...
**9. 主项目：表单验证 -** 在本节中，我们将进行表单验证，以防止提交无效数据。意外难免发生，我们的职责是确保检查错误。我们将学习如何针对各种场景实施验证规则。
...
**10. 主项目：身份验证 -** 在本部分，我们将开始进行身份验证和注册。用户注册帐户后即可立即登录。我们将使用 `Firebase` 作为后端解决方案来管理和验证用户。
...
**11. 主项目：路由 -** 在本节中，我们将使用 `Vue Router` 库来处理路由。该路由库底层使用 `History API` 来处理导航，从而允许我们在页面之间切换而无需刷新资源。我们将了解如何使用此库来帮助我们保护路由、添加元字段，并根据当前路由自定义网站的外观。
...
**12. 主项目：文件上传 -** 在本节中，我们将开始向 `Firebase` 上传文件。使用 `Firebase` 验证文件至关重要。我们不希望用户在听音乐时意外下载恶意文件。我们将介绍如何使用 `Firebase` 规则强制执行验证。之后，我们将把其他信息存储在数据库中。我们甚至还会介绍如何在拖放功能不可用时实现备用方案。
...
**13. 主项目：播放音乐 -** 在本节中，我们将开始实现音频播放器的功能。我们将实现播放/暂停、快进/快退和显示当前时长等基本功能。我们还将允许用户提交评论、浏览音乐和排序数据，从而使网站的其他功能也得以实现。
...
**14. 高级项目：指令 -** 在本节中，我们将简要学习如何编写自定义指令。指令是可以为元素添加额外属性和方法的属性。我们将介绍基础知识，并探讨修饰符和参数等高级概念。
...
**15. 高级项目：国际化 -** 在本节中，我们将为项目添加国际化功能，以便将项目翻译成多种语言。翻译消息的方法有很多种。有些消息需要格式化或使用复数形式。我们还可以翻译数字（例如货币）。我们将学习如何使用几行代码在不同语言之间切换。
...
**16. 主项目：PWA -** 网络已经转向移动优先策略。在本节中，我们将添加 `PWA`（**P**rogressive **W**eb **A**pp，渐进式 `Web` 应用）功能，通过使用 `Workbox` 缓存文件并使用 `Firebase` 持久化数据，使我们的应用更适合移动设备。
...
**17. 最终项目：优化 -** 在本节中，我们将对应用进行一些最终优化，然后再将其发布到生产环境。我们将介绍各种技术和工具，以帮助我们微调应用。我们可以使用 `Vite` 对文件进行分块并自动注册组件。为了提供友好的用户体验，我们将添加一些功能，帮助用户了解应用正在处理请求。

**18. MASTER PROJECT: DEPLOYMENT -** In this section, we'll deploy our app to `Vercel` with a single command! Quick and easy.
**18. 主项目：部署 -** 在本节中，我们将使用一条命令将我们的应用程序部署到 `Vercel`！快速简便。
...
**19. 主项目：测试 -** 在本节中，我们将讨论如何使用 `Vitest` 测试项目。单元测试允许我们隔离地测试某个功能。这是您编写的最常见的测试类型。测试中存在一些问题，可以通过模拟来解决。通过模拟代码，我们可以隔离功能并对其进行测试。之后，我们将学习快照测试和端到端测试，从而将所有内容整合起来。
...
**20. 组合式 API -** 在本节中，我们将讨论 `Vue` 最新引入的特性——组合式 `API`。它提供了一种不同的组件构建方式。虽然 `Vue` 提供了一个简单的组件编写 `API`，但它也存在一些不足之处。组合式 `API` 正是为了解决这些问题而生的。我们将探讨使用组合式 `API` 的原因，并解释它被引入的缘由。
...
这门课程的目的不是让你盲目地跟着代码写，而不去理解背后的原理，以至于课程结束后你除了去看其他教程之外什么都不知道该做什么。不！ **这门课程会激励你、挑战你，让你从 Vue 的零基础成长为 Vue 开发领域顶尖的 10% 人才。**
...
我们保证这是最全面的 `Vue JS` 在线课程！观看课程大纲视频，了解我们将涵盖的所有主题、我们将构建的所有项目，以及您将学习的所有技术，从而成为一名顶尖的 `Vue` 开发人员！
...
**安德烈·内戈伊** 是 **Udemy 上评分最高、发展最快的开发课程讲师之一。** 他的毕业生已进入苹果、谷歌、亚马逊、摩根大通、IBM、优衣库等全球顶尖科技公司工作。他曾在硅谷和多伦多担任高级软件开发人员多年，现在将自己积累的经验倾囊相授，教授编程技能，帮助你发现成为一名开发人员所能带来的无限职业机遇。
...
**路易斯·拉米雷斯二世** 是一位拥有十年 Web 应用程序开发经验的软件工程师。他毕生致力于构建可靠、久经考验、安全、可扩展且高性能的解决方案。作为一名工程师，他始终将开发能够提供流畅用户体验的应用程序作为首要任务。
...
大多数课程会教你“怎么做”，却从不讲“为什么”。 **路易斯认为，理解核心和关键的编程概念能让你在当今竞争激烈的工程师市场中脱颖而出。** 他将帮助你踏上这段旅程，为你揭开其他老师可能一带而过的底层概念的神秘面纱。
...
学生们将掌握批判性思维技能，并能与团队成员有效沟通，从而克服任何挑战。
...
- Learn to lead Vue projects by making good architecture decisions and helping others on your team
  学习如何领导 Vue 项目，做出良好的架构决策并帮助团队中的其他成员。
...
- Using the latest ES6/ES7/ES8/ES9/ES10/ES11 JavaScript to write clean code
  使用最新的 ES6/ES7/ES8/ES9/ES10/ES11 JavaScript 编写简洁的代码
- Learn why Vue is outgrowing React and Angular in developer popularity!
  了解为什么 Vue 在开发者中的受欢迎程度超过了 React 和 Angular！

#### Are there any course requirements or prerequisites? 是否有任何课程要求或先修课程？
...
- You do not need any experience with Vue or any other JS framework!
  你不需要任何 Vue 或其他 JS 框架的使用经验！

#### Who this course is for: 本课程适合以下人群：
...
- Students who want to gain experience working on a scalable large application
  希望获得开发可扩展大型应用程序经验的学生
- Bootcamp or online tutorial graduates that want to go beyond the basics
  参加过强化训练营或在线课程，并希望在基础知识之外继续深造的学员
